...
function manualControl(character){
  ...
  if(character.live){
    if(keyDown(control.left)||variable > 320){
      character.velocity.x-=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(-1);
    }

    if(keyDown(control.right)||variable < 320){
      character.velocity.x+=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(1);
    }
...
function jumping(character){
	if( ( (keyWentDown(control.up)|| variable2 < 200) &&character.live) || (touchIsDown&&character.live) ){
...
function checkStatus(character){    
  ...
  if(character.live==false && character.liveNumber==0){
    ...
    qwertyuiopasdfghjklzxcvbn.play()
